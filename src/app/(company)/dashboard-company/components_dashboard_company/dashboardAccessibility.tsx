...
import type { DashboardTone } from "@/features/company-dashboard/types";
...
export function DashboardToneTag({
  tone,
  label,
  statusText,
  style,
...
export function DashboardSectionHeading({
  ...
  return (
    <div style={{ marginBottom: 14 }}>
      <Typography.Title id={id} level={3} style={{ margin: 0 }}>
        {title}
      </Typography.Title>
...
export function DashboardEmptyText({ children }: { children: ReactNode }) {
  return (
    <Typography.Text style={{ color: "var(--raka-text-secondary)" }}>
      {children}
    </Typography.Text>
  );
}
