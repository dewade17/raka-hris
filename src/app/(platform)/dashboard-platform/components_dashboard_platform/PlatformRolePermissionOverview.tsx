...
export function PlatformRolePermissionOverview({ data }: PlatformRolePermissionOverviewProps) {
  ...
  return (
    <section id="roles-access" style={{ marginTop: 28, scrollMarginTop: 96 }}>
      <SectionHeading
        title="Roles & Access"
      ...
      <Row gutter={[16, 16]}>
        <Col xs={24} md={8}>
          <Metric title="Roles" value={data.totalRoles} icon={<KeyRound size={18} />} />
        </Col>
        <Col xs={24} md={8}>
          <Metric title="Permissions" value={data.totalPermissions} icon={<ShieldCheck size={18} />} />
        </Col>
        <Col xs={24} md={8}>
          <Metric title="Role assignments" value={data.roleAssignments} icon={<UsersRound size={18} />} />
        </Col>
      </Row>
